...
import { getPuestos } from "@/services/ticketService";
import FormularioPuestos from "@/components/form/example-form/FormulariosPuestos";
type SortKey = "label";
...
export default function TablePuestos() {
  ...
  const fetchData = async() => {
    try {
     ...
     console.log(result)
    if(result){
        setTableRowData(result.data.puestos)
    }
    } catch{
        setTableRowData([])
    }
                  ...
                  <TableRow key={index}>
                    ...
                    <TableCell className="px-7 py-4 font-normal text-gray-800 border border-gray-100 dark:border-white/[0.05] text-theme-sm dark:text-white/90 whitespace-nowrap">
                      <div className="flex items-center w-full gap-2">
                        <Tooltip
                          content="Editar Puesto de Trabajo"
                          position="top"
                          theme="dark"
                        >
                          <button
