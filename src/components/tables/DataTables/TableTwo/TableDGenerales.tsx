...
import { getDGenerales } from "@/services/ticketService";
import FormularioDGenerales from "@/components/form/example-form/FormularioDGenerales";
type SortKey = "label";
...
export default function TableDGenerales() {
  ...
  const fetchData = async() => {
    try {
     const result = await getDGenerales();
     console.log(result)
    if(result){
        setTableRowData(result.data.dgenerales)
    }
    } catch{
        setTableRowData([])
    }
    ...
    <>
      ...
      <Modal isOpen={isOpen} onClose={closeModal} className="max-w-[700px] m-4">
        <div className="no-scrollbar relative w-full max-w-[700px] max-h-[90vh] overflow-y-auto rounded-3xl bg-white p-4 dark:bg-gray-900 lg:p-4">
          <FormularioDGenerales
            dgeneral={singleItem ?? { value: "", label: "" }}
            disabled={disabled}
            isEdit={isEdit}
            isCreate={isCreate}
            onSuccess={fetchData}
